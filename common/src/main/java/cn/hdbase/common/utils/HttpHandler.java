package cn.hdbase.common.utils;
import cn.hdbase.common.config.SystemConfig;
import cn.hdbase.common.config.WeiXinConfig;
...
public class HttpHandler {
	...
	public String httpsPostToMch(String url,String content)throws Exception{
		String result = null;
		KeyStore keyStore  = KeyStore.getInstance("PKCS12");

		InputStream  instream = this.getClass().getResourceAsStream(WeiXinConfig.getInstance().getMchKeyStorePath());
		try {
			keyStore.load(instream, WeiXinConfig.getInstance().getMchId().toCharArray());
		} finally {
			instream.close();
		}
		// Trust own CA and all self-signed certs
		SSLContext sslcontext = SSLContexts.custom()
				.loadKeyMaterial(keyStore, WeiXinConfig.getInstance().getMchId().toCharArray())
				.build();
